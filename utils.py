...
from info import AUTH_CHANNEL, LONG_IMDB_DESCRIPTION, MAX_LIST_ELM, SHORTLINK_URL, SHORTLINK_API, IS_SHORTLINK, LOG_CHANNEL, TUTORIAL, GRP_LNK, CHNL_LNK, CUSTOM_FILE_CAPTION, SECOND_SHORTLINK_URL, SECOND_SHORTLINK_API, ADMINS, REQ_CHANNEL
...
async def get_poster(query, bulk=False, id=False, file=None):
    if not id:
        ...
        if year:
            ...
            year = None
        movieid = imdb.search_movie(title.lower(), results=15)
        if not movieid:
            return None
        if year:
